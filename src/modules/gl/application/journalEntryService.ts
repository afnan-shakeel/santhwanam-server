...
import { JournalEntry, JournalEntryLine, EntryStatus } from '../domain/entities';
...
export class JournalEntryService {
  ...
  async createJournalEntry(data: {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const lines = await this.journalEntryLineRepo.createMany(
        lineDataWithAccounts.map((lineData, index) => ({
   ...
   */
  async postJournalEntry(entryId: string, userId: string, tx?: any): Promise<JournalEntry> {
    const db = tx || prisma;
   ...
   */
  async getJournalEntryById(entryId: string): Promise<{ entry: JournalEntry; lines: JournalEntryLine[] }> {
   ...
   */
  async getJournalEntriesBySource(sourceModule: string, sourceEntityId: string): Promise<JournalEntry[]> {
