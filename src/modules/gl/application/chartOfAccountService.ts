...
import { ChartOfAccount, AccountType, NormalBalance, STANDARD_CHART_OF_ACCOUNTS } from '../domain/entities';
import { AppError } from '@/shared/utils/error-handling/AppError';
import prisma from '@/shared/infrastructure/prisma/prismaClient';

export class ChartOfAccountService {
  constructor(private chartOfAccountRepo: ChartOfAccountRepository) {}
   ...
   */
  async deactivateAccount(accountId: string, userId: string): Promise<ChartOfAccount> {
    const account = await this.chartOfAccountRepo.findById(accountId);
    if (!account) {
      throw new AppError('Account not found', 404);
    }
   ...
   * Get account by ID
   */
  async getAccountById(accountId: string): Promise<ChartOfAccount> {
    const account = await this.chartOfAccountRepo.findById(accountId);
    if (!account) {
      throw new AppError('Account not found', 404);
    }
   ...
   * Get active accounts
   */
  async getActiveAccounts(): Promise<ChartOfAccount[]> {
   ...
   * Get system accounts
   */
  async getSystemAccounts(): Promise<ChartOfAccount[]> {
   ...
   */
  async seedStandardAccounts(createdBy: string): Promise<ChartOfAccount[]> {
