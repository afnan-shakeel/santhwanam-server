...
import { JournalEntryRepository } from '../../domain/repositories';
import { JournalEntry, EntryStatus } from '../../domain/entities';

export class PrismaJournalEntryRepository implements JournalEntryRepository {
  async create(
    entry: Omit<JournalEntry, 'entryId' | 'entryNumber' | 'isBalanced' | 'createdAt' | 'updatedAt'>,
    tx?: any
  ...
  async update(entryId: string, data: Partial<JournalEntry>, tx?: any): Promise<JournalEntry> {
    const db = tx || prisma;
  ...
  async findById(entryId: string, tx?: any): Promise<JournalEntry | null> {
    const db = tx || prisma;
  ...
  async findByNumber(entryNumber: string, tx?: any): Promise<JournalEntry | null> {
    const db = tx || prisma;
    const entry = await db.journalEntry.findUnique({
      where: { entryNumber },
      include: { lines: true },
  ...
  async findAll(tx?: any): Promise<JournalEntry[]> {
    const db = tx || prisma;
    const entries = await db.journalEntry.findMany({
      orderBy: { entryDate: 'desc' },
    });

    return entries.map((entry: any) => this.mapToDomain(entry));
  }

  async findByStatus(status: EntryStatus, tx?: any): Promise<JournalEntry[]> {
    const db = tx || prisma;
    const entries = await db.journalEntry.findMany({
  ...
  async findByDateRange(startDate: Date, endDate: Date, tx?: any): Promise<JournalEntry[]> {
    const db = tx || prisma;
    const entries = await db.journalEntry.findMany({
      where: {
        entryDate: {
  ...
  async findByAccount(accountId: string, tx?: any): Promise<JournalEntry[]> {
    const db = tx || prisma;
    const entries = await db.journalEntry.findMany({
      where: {
        lines: {
  ...
  async findBySource(sourceModule: string, sourceEntityId: string, tx?: any): Promise<JournalEntry[]> {
    const db = tx || prisma;
    const entries = await db.journalEntry.findMany({
      where: {
        sourceModule,
  ...
  async findUnbalanced(tx?: any): Promise<JournalEntry[]> {
    const db = tx || prisma;
    const entries = await db.journalEntry.findMany({
  ...
  async post(entryId: string, userId: string, tx?: any): Promise<JournalEntry> {
    const db = tx || prisma;
    const updated = await db.journalEntry.update({
      where: { entryId },
      data: {
  ...
  async reverse(entryId: string, userId: string, reason: string, tx?: any): Promise<JournalEntry> {
    const db = tx || prisma;
    const updated = await db.journalEntry.update({
      where: { entryId },
      data: {
  ...
  private mapToDomain(prismaEntry: any): JournalEntry {
    return {
      ...
      entryStatus: prismaEntry.entryStatus as EntryStatus,
