...
export function createMembersRouter(controller) {
    ...
    router.post("/:memberId/complete/personal-details", controller.completePersonalDetailsStep);
    // ===== STEP 2: NOMINEES =====
    router.post("/:memberId/nominees", validateBody(addNomineeSchema), controller.addNominee);
    router.get("/:memberId/nominees", controller.getNominees);
    router.patch("/:memberId/nominees/:nomineeId", validateBody(updateNomineeSchema), controller.updateNominee);
    router.delete("/:memberId/nominees/:nomineeId", controller.removeNominee);
    router.post("/:memberId/complete/nominees", controller.completeNomineesStep);
    // ===== STEP 3: DOCUMENTS & PAYMENT =====
    router.post("/:memberId/documents", validateBody(uploadMemberDocumentSchema), controller.uploadDocument);
    router.get("/:memberId/documents", controller.getDocuments);
    router.delete("/:memberId/documents/:documentId", controller.removeDocument);
    router.post("/:memberId/payment", validateBody(recordRegistrationPaymentSchema), controller.recordPayment);
    router.get("/:memberId/payment", controller.getPayment);
    // ===== SUBMISSION =====
    router.post("/:memberId/submit", controller.submitRegistration);
    // ===== MEMBER MANAGEMENT =====
    router.post("/:memberId/suspend", validateBody(suspendMemberSchema), controller.suspendMember);
